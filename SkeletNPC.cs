...
public sealed class SkeletNPC : Component, IMyDamageable
{
	// =========================
	// BASIC
	// =========================
	...
	[Property] public float Health { get; set; } = 110f;
	[Property] public float MaxHealth { get; set; } = 110f;

	[Property] public float AttackRange { get; set; } = 95f;
	[Property] public float AttackCooldown { get; set; } = 2.0f;
	[Property] public int AttackDamage { get; set; } = 15;

	[Property] public float MoveSpeed { get; set; } = 190f;
	...
	// ATTACK TIMING
	// =========================
	[Property, Group( "Attack Timing" )] public float AttackDuration { get; set; } = 1.15f;
	[Property, Group( "Attack Timing" )] public float HitDelay { get; set; } = 0.35f;

	// =========================
	// TURN (FIX "ICE")
	// =========================
	[Property, Group( "Turn" )] public float TurnSpeed { get; set; } = 22f;
	...
	{
		"Zombie_Attack", "Zombie_Attack2", "Zombie_Attack3"
	};
	...
	[Property, Group( "Animation (Sequence)" )] public float RunPlaybackRate { get; set; } = 0.85f;
	[Property, Group( "Animation (Sequence)" )] public float AttackPlaybackRate { get; set; } = 1.1f;
	...
	[Property] public Color OutlineHighHP { get; set; } = Color.Cyan;
	...
	// =========================
	// SKELET FEATURE: CRIT HIT
	// =========================
	[Property, Group( "Skelet" )] public float CritChance { get; set; } = 0.15f;
	[Property, Group( "Skelet" )] public float CritMultiplier { get; set; } = 1.8f;
	[Property, Group( "Skelet" )] public SoundEvent CritSound { get; set; }
	...
	protected override void OnStart()
	{
		...
		if ( DebugMode )
			Log.Info( $"💀 Skelet Nav init. Agent={(Agent != null ? "OK" : "MISSING")} Renderer={(ModelRenderer != null ? "OK" : "MISSING")}" );
	}
	...
	private void StartAttack()
	{
		...
		string seq = PickRandom( AttackSequences, fallback: "Zombie_Attack" );
	...
	private void TryDealDamage()
	{
		...
		int dmg = EffectiveAttackDamage;

		// 👇 Crit (фишка скелета)
		if ( Roll( CritChance ) )
		{
			dmg = Math.Max( 1, (int)MathF.Round( dmg * MathF.Max( 1f, CritMultiplier ) ) );
			if ( CritSound != null )
				Sound.Play( CritSound, Transform.Position );
		}
	...
	private void ApplySeparation()
	{
		...
		var all = Scene.GetAllComponents<SkeletNPC>();
		if ( all == null ) return;
	...
	private void Die()
	{
		...
		// Награда: скелет поскромнее мумии
		int expAmount = Rng.Next( 1, 4 ); // 1..3
	...
	private static bool Roll( float chance01 )
	{
		if ( float.IsNaN( chance01 ) || float.IsInfinity( chance01 ) ) return false;
