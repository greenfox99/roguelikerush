...
public sealed class DemonNPC : Component, IMyDamageable
{
	// =========================
	// BASIC
	// =========================
	...
	[Property] public float Health { get; set; } = 300f;
	[Property] public float MaxHealth { get; set; } = 300f;

	[Property] public float AttackRange { get; set; } = 125f;
	[Property] public float AttackCooldown { get; set; } = 1.6f;
	[Property] public int AttackDamage { get; set; } = 35;
	...
	[Property, Group( "Attack Timing" )] public float HitDelay { get; set; } = 0.30f;

	// =========================
	// TURN
	// =========================
	[Property, Group( "Turn" )] public float TurnSpeed { get; set; } = 24f;
	[Property, Group( "Turn" )] public float TurnSnapAngle { get; set; } = 75f;
	[Property, Group( "Turn" )] public float TurnVelocityMin { get; set; } = 18f;

	// =========================
	// PERF
	// =========================
	[Property, Group( "Perf" )] public float PlayerSearchInterval { get; set; } = 0.35f;
	...
	[Property, Group( "Nav" )] public float RepathInterval { get; set; } = 0.12f;
	[Property, Group( "Nav" )] public float StopDistance { get; set; } = 85f;

	// Разброс цели (чтобы не все бежали в одну точку)
	[Property, Group( "Nav" )] public float TargetOffsetRadius { get; set; } = 85f;
	[Property, Group( "Nav" )] public float TargetOffsetRefresh { get; set; } = 0.45f;

	// Separation (мягкое расталкивание)
	[Property, Group( "Nav" )] public bool SeparationEnabled { get; set; } = true;
	[Property, Group( "Nav" )] public float SeparationRadius { get; set; } = 110f;
	[Property, Group( "Nav" )] public float SeparationStrength { get; set; } = 95f;
	...
	[Property, Group( "Nav" )] public float StuckMoveEpsilon { get; set; } = 10f;
	[Property, Group( "Nav" )] public float StuckNudgeDistance { get; set; } = 240f;
	...
	[Property, Group( "Animation (Sequence)" )] public float AttackPlaybackRate { get; set; } = 1.2f;
	...
	[Property] public Color OutlineHighHP { get; set; } = Color.Red;
	[Property] public Color OutlineMediumHP { get; set; } = new Color( 1.0f, 0.45f, 0.0f );
	[Property] public Color OutlineLowHP { get; set; } = new Color( 0.35f, 0.0f, 0.0f );
	[Property] public float OutlineWidth { get; set; } = 2.6f;

	[Property] public float FadeOutTime { get; set; } = 2.4f;
	...
	protected override void OnStart()
	{
		...
		if ( DebugMode )
			Log.Info( $"😈 Demon Nav init. Agent={(Agent != null ? "OK" : "MISSING")} Renderer={(ModelRenderer != null ? "OK" : "MISSING")}" );
	}
	...
	private void ApplySeparation()
	{
		...
		var all = Scene.GetAllComponents<DemonNPC>();
		if ( all == null ) return;
	...
	private void Die()
	{
		...
		int expAmount = Rng.Next( 6, 12 ); // 6..11
