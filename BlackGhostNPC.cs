...
public sealed class BlackGhostNPC : Component, IMyDamageable
{
	// =========================
	// BASIC
	// =========================
	...
	[Property] public float Health { get; set; } = 180f;
	[Property] public float MaxHealth { get; set; } = 180f;

	[Property] public float AttackRange { get; set; } = 240f;
	...
	[Property] public int AttackDamage { get; set; } = 28;

	[Property] public float MoveSpeed { get; set; } = 340f;
	...
	// =========================
	[Property, Group( "Ranged" )] public bool UseProjectileAttack { get; set; } = true;
	[Property, Group( "Ranged" )] public GameObject ProjectilePrefab { get; set; }
	[Property, Group( "Ranged" )] public float ProjectileSpeed { get; set; } = 820f;
	[Property, Group( "Ranged" )] public float ProjectileLifetime { get; set; } = 3.0f;

	[Property, Group( "Ranged" )] public float AimZOffset { get; set; } = 42f;
	[Property, Group( "Ranged" )] public Vector3 ProjectileSpawnOffset { get; set; } = new Vector3( 42f, 0f, 42f );
	[Property, Group( "Ranged" )] public float MinThrowDistance { get; set; } = 90f;

	// =========================
	// BLACK GHOST EXTRA
	// =========================
	[Property, Group( "Black Ghost" )] public bool TintModelBlack { get; set; } = true;
	[Property, Group( "Black Ghost" )] public Color BlackGhostTint { get; set; } = new Color( 0.12f, 0.12f, 0.16f, 1.0f );
	[Property, Group( "Black Ghost" )] public bool StrongerOutline { get; set; } = true;
	[Property, Group( "Black Ghost" )] public Color OutlineHighColor { get; set; } = new Color( 0.45f, 0.20f, 1.00f, 1f );
	[Property, Group( "Black Ghost" )] public Color OutlineMidColor { get; set; } = new Color( 0.90f, 0.25f, 0.95f, 1f );
	[Property, Group( "Black Ghost" )] public Color OutlineLowColor { get; set; } = new Color( 1.00f, 0.15f, 0.25f, 1f );
	...
	// ATTACK TIMING
	// =========================
	[Property, Group( "Attack Timing" )] public float AttackDuration { get; set; } = 0.95f;
	[Property, Group( "Attack Timing" )] public float HitDelay { get; set; } = 0.22f;

	// =========================
	// TURN
	// =========================
	[Property, Group( "Turn" )] public float TurnSpeed { get; set; } = 24f;
	[Property, Group( "Turn" )] public float TurnSnapAngle { get; set; } = 80f;
	[Property, Group( "Turn" )] public float TurnVelocityMin { get; set; } = 25f;

	// =========================
	// PERF
	// =========================
	[Property, Group( "Perf" )] public float PlayerSearchInterval { get; set; } = 0.40f;
	...
	[Property, Group( "Nav" )] public float RepathInterval { get; set; } = 0.16f;
	[Property, Group( "Nav" )] public float StopDistance { get; set; } = 110f;

	[Property, Group( "Nav" )] public float TargetOffsetRadius { get; set; } = 85f;
	[Property, Group( "Nav" )] public float TargetOffsetRefresh { get; set; } = 0.50f;

	[Property, Group( "Nav" )] public bool SeparationEnabled { get; set; } = true;
	[Property, Group( "Nav" )] public float SeparationRadius { get; set; } = 100f;
	[Property, Group( "Nav" )] public float SeparationStrength { get; set; } = 82f;
	[Property, Group( "Nav" )] public float SeparationMaxStep { get; set; } = 1.0f;

	[Property, Group( "Nav" )] public bool AntiStuckEnabled { get; set; } = true;
	[Property, Group( "Nav" )] public float StuckCheckInterval { get; set; } = 0.45f;
	[Property, Group( "Nav" )] public float StuckMoveEpsilon { get; set; } = 14f;
	[Property, Group( "Nav" )] public float StuckNudgeDistance { get; set; } = 220f;
	...
	[Property, Group( "Animation (Sequence)" )] public float RunPlaybackRate { get; set; } = 0.95f;
	[Property, Group( "Animation (Sequence)" )] public float AttackPlaybackRate { get; set; } = 1.20f;
	...
	[Property] public Color OutlineHighHP { get; set; } = new Color( 0.45f, 0.20f, 1.00f, 1f );
	[Property] public Color OutlineMediumHP { get; set; } = new Color( 0.90f, 0.25f, 0.95f, 1f );
	[Property] public Color OutlineLowHP { get; set; } = new Color( 1.00f, 0.15f, 0.25f, 1f );
	[Property] public float OutlineWidth { get; set; } = 2.8f;
	...
	protected override void OnStart()
	{
		...
		if ( ModelRenderer != null && ModelRenderer.IsValid && TintModelBlack )
			ModelRenderer.Tint = BlackGhostTint;

		if ( StrongerOutline )
		{
			OutlineHighHP = OutlineHighColor;
			OutlineMediumHP = OutlineMidColor;
			OutlineLowHP = OutlineLowColor;
		}
	...
	private void ThrowProjectileAtPlayer()
	{
		...
		{
			if ( DebugMode ) Log.Warning( $"[BlackGhostNPC] ProjectilePrefab не назначен на '{GameObject?.Name}'" );
			return;
		}
		...
		projGO.Name = "BlackGhostProjectile";

		var proj = projGO.Components.Get<Projectile>( FindMode.EnabledInSelfAndDescendants );
		if ( proj == null )
		{
			if ( DebugMode ) Log.Warning( $"[BlackGhostNPC] В ProjectilePrefab нет компонента Projectile.cs (или он Disabled)." );
			return;
		}
	...
	private void ApplySeparation()
	{
		...
		var allGhosts = Scene.GetAllComponents<NPC>();
		var allBlackGhosts = Scene.GetAllComponents<BlackGhostNPC>();
		...
		if ( allBlackGhosts != null )
		{
			foreach ( var other in allBlackGhosts )
			{
				if ( other == null || !other.IsValid ) continue;
	...
	private void Die()
	{
		...
		if ( DeathEffectPrefab != null )
		{
			...
			effect.Name = "BlackGhostDeathEffect";
		}
		...
		if ( dropper == null && DebugMode )
			Log.Warning( $"[BlackGhostNPC] LootDropper не найден на '{GameObject?.Name}'. Добавь компонент LootDropper в префаб NPC." );
	...
	private async Task FadeOutAndDestroy( float duration )
	{
		...
		while ( time < duration )
		{
			...
			if ( ModelRenderer != null )
			{
				var fadeColor = TintModelBlack ? BlackGhostTint : Color.White;
				ModelRenderer.Tint = new Color( fadeColor.r, fadeColor.g, fadeColor.b, alpha );
			}
